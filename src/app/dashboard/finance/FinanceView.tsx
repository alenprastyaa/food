...
const RANGES = [
  { key: "day", label: "Hari Ini", jp: "今日" },
  { key: "week", label: "7 Hari", jp: "週間" },
  { key: "month", label: "30 Hari", jp: "月間" },
];
...
export default function FinanceView({ role, outlets }: { role: string; outlets: { id: string; name: string }[] }) {
  ...
  return (
    <div>
      <PageHeader title="Keuangan" jp="会計" subtitle={role === "OWNER" ? "Laporan seluruh outlet" : "Laporan outletmu"} />
      ...
      <div className="p-5 lg:p-8 space-y-5">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex gap-1.5 bg-paper rounded-full p-1 ring-1 ring-sumi/10">
            {RANGES.map((r) => (
              <button key={r.key} onClick={() => setRange(r.key)} className={`rounded-full px-4 py-1.5 text-sm font-round font-bold transition ${range === r.key ? "bg-shu text-white" : "text-sumi/60"}`}>
                {r.label}
              </button>
            ))}
          </div>
        ...
        {loading || !data ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
              <Card label="Revenue" jp="売上" value={rupiah(data.revenue)} accent big />
              <Card label="Total Order" jp="注文" value={data.orderCount.toString()} />
              <Card label="Pembayaran" jp="支払" value={data.paidCount.toString()} />
              <Card label="Dibatalkan" jp="取消" value={data.cancelled.toString()} tone="text-rose-500" />
              <Card label="Rata-rata" jp="平均" value={rupiah(data.avgOrder)} />
            </div>
            ...
            <div className="paper-card rounded-2xl overflow-hidden">
              <div className="px-5 py-3 border-b border-sumi/10 flex items-center justify-between">
                <h3 className="font-display font-extrabold">Rincian Transaksi</h3>
                <span className="text-xs text-sumi/40">{data.orders.length} transaksi</span>
              </div>
              {data.orders.length === 0 ? (
                <EmptyState icon="📊" title="Belum ada transaksi" subtitle="Coba ubah rentang waktu." />
              ) : (
                      ...
                      {data.orders.map((o) => {
                        ...
                        return (
                          <tr key={o.id} className="border-b border-sumi/5 hover:bg-washi/50">
                            <td className="px-5 py-2.5 font-round font-bold text-sumi">{o.invoiceNumber}</td>
                            ...
                            <td className="px-5 py-2.5 text-right font-display font-extrabold text-shu">{rupiah(o.total)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
...
function Card({ label, jp, value, accent, tone, big }: { label: string; jp: string; value: string; accent?: boolean; tone?: string; big?: boolean }) {
  return (
    <div className={`paper-card rounded-2xl p-4 relative overflow-hidden ${big ? "col-span-2 lg:col-span-1" : ""}`}>
      ...
      <p className={`mt-1 font-display font-extrabold ${big ? "text-2xl" : "text-xl"} ${tone ?? (accent ? "text-shu" : "text-sumi")}`}>{value}</p>
    </div>
  );
}
